...
from controller.usercontroller import get_pdf, getusermotard, delete_user,create_user, update_user,update_imageuser,verifieremail, login,getuserid,getuser, get_role, create_role, update_role
from controller.arretcontroller import create_arret, update_arret, delete_arret, getarret
from controller.categoriecontroller import delete_categorie, create_categorie, update_categorie, getcategorie
from controller.itinerairecontroller import delete_itineraire, create_itineraire, update_itineraire, getitineraire
from controller.vehiculecontroller import create_vehicule,delete_vehicule, update_vehicule, getvehicule
from controller.tarificationcontroller import delete_tarification, create_tarification, gettarification, update_tarification
from controller.coursecontroller import delete_course, getcourseid, create_course, getcourses, update_course
...
from controller.justificationcontroller import create_justification, getjustification, gettypejust,getjustificationid
from controller.emailcontroller import send_email

#IP_ADDRESS = '192.168.38.147' "msg": "Token has expired" 
IP_ADDRESS = '192.168.88.147'  
PORT = 1200
...
@app.route('/api/getusermotard', methods=['GET'])
#@jwt_required()
def api_getusermotard():
    return (getusermotard())  
...
@app.route('/api/delete_categorie/<int:id>', methods=['DELETE'])
...
@app.route('/api/delete_itineraire/<int:id>', methods=['DELETE'])
def api_delete_itineraire(id):
      return (delete_itineraire(id))
...
@app.route('/api/delete_vehicule/<int:id>', methods=['DELETE'])
def api_delete_vehicule(id):
      return (delete_vehicule(id))
...
@app.route('/api/delete_tarification/<int:id>', methods=['DELETE'])
def api_delete_tarification(id):
      return (delete_tarification(id))
...
@app.route('/api/getjustificationid/<int:id>', methods=['GET'])
def api_getjustificationid(id):
    return (getjustificationid(id))
